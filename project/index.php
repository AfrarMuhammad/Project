...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'reserve') {
    ...
    echo "<pre class='debug'>Received POST data: " . htmlspecialchars($postData) . "</pre>";
    ...
    echo "<pre class='debug'>Input values: email=$email, jewelry_id=$jewelry_id, date=$reservation_date, time_slot=$time_slot_only, special_requests=$special_requests</pre>";

    if (!$email || !$jewelry_id || !$reservation_date || !$time_slot_only) {
        $_SESSION['error'] = "Missing required fields: email=$email, jewelry_id=$jewelry_id, date=$reservation_date, time_slot=$time_slot_only";
    ...
    if ($user_id_stmt === false) {
        $_SESSION['error'] = "Prepare failed (user_id fetch): " . $conn->error;
        header("Location: index.php");
        exit();
    }
    ...
    if ($user_id_result->num_rows === 0) {
        $_SESSION['error'] = "User not found for email: $email";
        header("Location: index.php");
        exit();
    }
    ...
    try {
        $check_stmt = $conn->prepare("SELECT name, status, stock, description FROM jewelry WHERE id = ? AND status = 'available' AND stock > 0");
        if ($check_stmt === false) {
        ...
        if ($result->num_rows === 0) {
            throw new Exception("Item not available or out of stock for jewelry_id: $jewelry_id");
        }
        $item = $result->fetch_assoc();
        $stock = $item['stock'];
        $item_name = $item['name'];
        $description = $item['description'];
        error_log("Jewelry check passed: name=$item_name, stock=$stock");
        ...
        if ($check_reservation_stmt === false) {
            throw new Exception("Prepare failed (reservation check): " . $conn->error);
        }
        ...
        error_log("Reservation check passed: no conflicts");
        ...
        if (!$stmt->execute()) {
            throw new Exception("Insert failed: " . $conn->error . " (Query: INSERT INTO reservations (user_id, jewelry_id, reservation_date, time_slot, special_requests) VALUES ($user_id, $jewelry_id, '$reservation_date', '$time_slot_only', '$special_requests'))");
        }
        error_log("Reservation inserted successfully");

        $new_stock = $stock - 1;
        $update_stmt = $conn->prepare("UPDATE jewelry SET stock = ?, status = IF(?, 'reserved', status) WHERE id = ?");
        ...
        error_log("Jewelry stock updated: new_stock=$new_stock");

        if ($new_stock <= 0) {
            $conn->query("UPDATE jewelry SET status = 'out of stock' WHERE id = $jewelry_id");
            error_log("Stock depleted, status set to 'out of stock'");
        }
...
// Fetch available jewelry and pre-select if ID is provided (no longer needed for static content, but kept for reservation logic)
$selected_jewelry_id = isset($_GET['id']) ? (int)$_GET['id'] : null;
// $jewelry = $conn->query("SELECT * FROM jewelry WHERE status = 'available' AND stock > 0"); // Commented out since using static HTML
?>
    ...
    <script>
        ...
        document.querySelectorAll('.collection-item').forEach(item => {
            item.addEventListener('click', function() {
                const reservationForm = document.getElementById('reservation-form');
                document.getElementById('jewelry_id').value = this.dataset.id;
                const descriptions = {
                    '1': 'A stunning emerald necklace featuring a 2-carat center stone surrounded by diamonds',
                    '2': 'Elegant sapphire earrings with white gold setting',
                    '3': 'Bold ruby ring with diamond accents',
                ...
                const description = descriptions[this.dataset.id] || 'No description available';
                document.getElementById('description').value = description;
                document.getElementById('item-description').textContent = description;
        ...
        document.querySelectorAll('.time-slot').forEach(slot => {
            slot.addEventListener('click', function() {
                ...
                document.querySelector('input[name="time_slot"]').value = this.dataset.value;
            });
        });
        ...
        document.querySelector('form').appendChild(timeSlotInput);
    </script>
</body>
</html>
